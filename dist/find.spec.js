...
function test(highestPattern, code) {
    expect((0, find_1.findHighestPattern)(SWC.parseSync(code, { target: "es2022", syntax: "ecmascript" }))).toBe(highestPattern);
}
describe("Find top-level await usage in module", () => {
    it("should work with top-level await", () => {
        test(find_1.CodePattern.TopLevelAwait, `
    ...
    it("should work with dynamic import but without top-level await", () => {
        test(find_1.CodePattern.DynamicImport, `
      function qwqwq() {
        return import("qwq");
      }
    `);
